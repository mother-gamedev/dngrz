...
static func is_strike(pitch_position: Vector3) -> bool:
	...
	var in_horizontal := absf(pitch_position.x) <= half_width
	var in_vertical := pitch_position.y >= FieldConstants.STRIKE_ZONE_BOTTOM and pitch_position.y <= FieldConstants.STRIKE_ZONE_TOP
